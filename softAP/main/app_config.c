...
provison_type_t provision_type =PROVISION_ACCESSPOINT;
...
void ap_start(){
    ...
    // if (strlen(wifi_config.ap.password[0]) == 0) {
...
void app_config1(void){
    ...
    if(!provisioned){
        ...
        esp_smartconfig_stop();
         ...
         ap_start();
         start_webserver();
         http_post_set_callback(hppt_sever_data_callback);
         xEventGroupWaitBits(s_wifi_event_group, HTTP_CONFIG_DONE, true, false, portMAX_DELAY); 
         stop_webserver();
